...
from resnet import Dilated_ResNet_101
import pyramid_pooling2 as pp
...
SNAPSHOT_DIR = '../../../data/redbull_ckpt/20170711'
WEIGHT_DECAY = 0.0001
CROP_SIZE = 512
SCALE_SIZE = 544
...
def main():
    ...
    with tf.variable_scope('', reuse=True):
        net075 = Dilated_ResNet_101({'data': image_batch075}, is_training=args.is_training)
    with tf.variable_scope('', reuse=True):
        net05 = Dilated_ResNet_101({'data': image_batch05}, is_training=args.is_training)
    ...
    # Predictions.
    res_output100 = net.layers['res5c_relu']
    res_output075 = net075.layers['res5c_relu']
    res_output05 = net05.layers['res5c_relu']
    res_output = tf.reduce_max(tf.stack([res_output100,
                                         tf.image.resize_images(res_output075, tf.shape(res_output100)[1:3,]),
                                         tf.image.resize_images(res_output05, tf.shape(res_output100)[1:3,])]), axis=0)

    # print('net.layers[fc1_voc12]:', net.layers['fc1_voc12'].shape)

    with tf.variable_scope('', reuse=False):
        pooled_output100 = pp.pyramid_pooling_module(res_output100, original_HW=(CROP_SIZE/8, CROP_SIZE/8))
        fc100 = pp.fully_connected(pooled_output100, num_classes=args.num_classes , is_training=args.is_training)
    with tf.variable_scope('', reuse=True):
        pooled_output075 = pp.pyramid_pooling_module(res_output075, original_HW=(CROP_SIZE*3/32, CROP_SIZE*3/32))
        fc075 = pp.fully_connected(pooled_output075, num_classes=args.num_classes , is_training=args.is_training)
    with tf.variable_scope('', reuse=True):
        pooled_output05 = pp.pyramid_pooling_module(res_output05, original_HW=(CROP_SIZE/16, CROP_SIZE/16))
        fc05 = pp.fully_connected(pooled_output05, num_classes=args.num_classes , is_training=args.is_training)
    with tf.variable_scope('', reuse=True):
        pooled_output_max = pp.pyramid_pooling_module(res_output, original_HW=(CROP_SIZE/8, CROP_SIZE/8))
        fc_max = pp.fully_connected(pooled_output_max, num_classes=args.num_classes , is_training=args.is_training)
    
    raw_output = tf.reduce_mean(tf.stack([fc100,
                                          fc_max,
                                          tf.image.resize_images(fc075, tf.shape(fc100)[1:3,]),
                                          tf.image.resize_images(fc05, tf.shape(fc100)[1:3,])]),
                                axis=0, 
                                name="final_output")
    ...
    print("Number of trainable objects:", len(all_trainable))
